...
def etl_sql_nosql():
    ...
    def t4_integrar(sql_json: str, costos_json: str, bigmac_json: str) -> str:
        ...
        # ESTRATEGIA:
        # - region, capital, poblacion, continente  → vienen de MongoDB (están completos)
        # - tasa_de_envejecimiento                  → viene del CSV/SQLite (solo campo útil)
        # El CSV tiene 101/106 filas con NULL en capital/region/poblacion,
        # por eso usamos MongoDB como fuente principal de esos campos.

        # Del CSV solo necesitamos nombre_pais y tasa_de_envejecimiento
        ...
        # Merge 2: resultado + solo tasa de envejecimiento del CSV
        df_final = pd.merge(
            df_temp,
            df_sql_slim,
            left_on="pais",
        ...
        df_final.drop(columns=["nombre_pais"], errors="ignore", inplace=True)

        # Renombrar columnas de MongoDB al esquema final del DW
        df_final.rename(columns={
            "continente_mongo": "continente",
            "region":           "region",
            "capital":          "capital",
            "poblacion":        "poblacion",
